...
var emailType = (props, propName, componentName) => {
  warning(
    ...
    `Invalid '${propName}' sent to 'Gravatar'. Check the render method of '${componentName}'.`
  );
};
...
var sizeType = (props, propName, componentName) => {
  warning(
    ...
    `Invalid '${propName}'.  '${props[propName]}' cannot be parsed as an Integer.  Please check the render method of '${componentName}'`
  );
}
...
var App = React.createClass({
  render () {
    var users = this.props.users.map((user) => {
      return (
        <li key={user.id}>
          <Gravatar loginId={user.email} size={"34"} /> {user.name}
        </li>
      );
    });
    return (
